function plotgame(x1,y1,z1,x2,y2,z2,thisDelay)
    ...
    p1.getObject().setZData(z1(1:2));
    p2.getObject().setXData(x2(1:2));
    p2.getObject().setYData(y2(1:2));
    p2.getObject().setZData(z2(1:2));
...
%     writer.add();
    ...
    t = timer('ExecutionMode','fixedSpacing', 'Period',DELAY, 'TimerFcn', {@localTimer, p1.getObject(), p2.getObject(), x1, y1, z1, x2, y2, z2});
%    t = timer('ExecutionMode','fixedSpacing', 'Period',DELAY, 'TimerFcn', {@localTimer, p1.getObject(), x, y, writer.Object});
    start(t);

    function localTimer(t, EventData, p1, p2, x1, y1, z1, x2, y2, z2)
%    function localTimer(t, EventData, p1, x, y, wr)
        ...
        elseif k > 1
            ...
            p1.setZData(z1(k-1:k));
            p1.plotRedraw();
            p2.setXData(x2(k-1:k));
            p2.setYData(y2(k-1:k));
            p2.setZData(z2(k-1:k));
            p2.plotRedraw();
%	    wr.add();
        end
    end  % localTimer
