function [Angle, iSt, iEnd, iMax] = PlotAngleError(tOn,tEnd,xyz,t)
...
% iSt = iSt + tOn-1;
ups = find(~iLogic,3,'first') +tOn-1;  %% start at first pull up, (3rd rising z point)
if numel(ups)==0
  iSt=tOn;
else
  iSt = ups(end);
...
iEnd = tEnd;
iRange = [zeros(1,iSt-tOn) ones(1,iEnd-iSt+1)];
