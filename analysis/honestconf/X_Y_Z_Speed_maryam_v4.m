function X_Y_Z_Speed_maryam_v4(subj1,subj2,minz)
...
%         clear multmxback
        ...
        launchpointsInd{isubj}=NaN(size(hitgamereg{isubj}));
        hitpointsInd{isubj}=NaN(size(hitgamereg{isubj}));
        molelistind{isubj}=NaN(length(hitgamereg{isubj}),7);
        datalaunchhit{isubj,trial}=NaN(length(hitgamereg{isubj}),17);
        
        %subject a
        %determining target hit and launch point for each hit
        for ihit =1:length(hitgamereg{isubj})
            
            
            curhitind=lochitgamereg{isubj}(ihit);
            framesbefore=min(240,curhitind-1);
            framesafter=max(120,length(Z{isuj}));
%             ***********************
%             Iamhere
%             **********************
